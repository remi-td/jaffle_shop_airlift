...
(
    ...
    (
        ...
        from al210ghxMjTZ.sum_customer_order
        where valid_period contains current_timestamp
        qualify prod_rank <=3
    ) o
    join al210ghxMjTZ.sum_product p
...
(
    ...
    (
        ...
        from al210ghxMjTZ.sum_product p
        cross join "al210ghxMjTZ"."disc_customers" c
        where p.valid_period contains current_timestamp
        --...that have not yet been seen in orders
        and not exists
        (sel 1 from al210ghxMjTZ.sum_customer_order o where o.product_cd=p.product_cd and  c.customer_key=o.customer_key)
        qualify prod_rank <=3
    ) p
    group by 1    
)
...
,final as (
    ...
    from "al210ghxMjTZ"."disc_customers" customer
